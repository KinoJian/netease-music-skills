...
`ncm-cli tui` 是**交互式终端应用**，需要真实 TTY，无法在 Agent/Bash 工具中直接运行。**必须先启动 TUI 窗口**，TUI 是播放服务端，后续所有播放命令都是发送给它的客户端指令。

### Windows 启动 TUI

```bash
# 1. 写临时脚本（避免引号转义问题）
cat > $env:TEMP\launch_ncm.ps1 << 'EOF'
$env:PATH = 'C:\Program Files\MPV Player;' + [Environment]::GetEnvironmentVariable('PATH', 'User') + ';' + [Environment]::GetEnvironmentVariable('PATH', 'Machine')
ncm-cli tui
EOF

# 2. 用 Windows Terminal 启动
wt.exe --title "网易云音乐" powershell -NoExit -File "$env:TEMP\launch_ncm.ps1"
```

### TUI 运行后的播放方式

TUI 运行期间**必须用 `queue add` 而非 `play`**：

```bash
# 添加到队列末尾
ncm-cli queue add --encrypted-id <加密ID> --original-id <原始ID>
...
| 问题 | 原因 | 解决 |
|------|------|------|
| "TUI 必须在交互式终端中运行" | 在 Agent/Bash 中直接调用 | 用 wt.exe 新窗口启动 |
| "歌曲已发送到播放服务"但未播放 | TUI 未启动，无服务端 | 先启动 TUI |
| queue 中显示加密ID而非歌名 | 正常现象，歌曲仍可播放 | 不影响播放 |
| "TUI 正在运行，请使用 queue add" | TUI 运行中不能用 play | 改用 queue add |
